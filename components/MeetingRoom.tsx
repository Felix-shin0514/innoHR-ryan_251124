...
const MeetingRoom: React.FC<MeetingRoomProps> = ({ user }) => {
  ...
  const [bookings, setBookings] = useState<Booking[]>(() => {
    ...
    if (!saved) {
      ...
      return [
        { id: 'b1', roomId: 'r1', date: todayStr, time: '10:00', userId: 'other1', userName: '박팀장', department: '영업팀', content: '주간 영업 회의' },
        { id: 'b2', roomId: 'r2', date: todayStr, time: '14:00', userId: 'other2', userName: '김대리', department: '개발팀', content: '프로젝트 기획 미팅' },
        { id: 'b3', roomId: 'r3', date: todayStr, time: '09:00', userId: 'other3', userName: '최사원', department: '인사팀', content: '면접 진행' },
      ];
    }
  ...
  const handleSlotClick = (roomId: string, roomName: string, time: string, isPast: boolean) => {
    ...
    if (existingBooking) {
      ...
      setBookingDept(user.department);
      setBookingContent('');
  ...
  const handleConfirmAction = () => {
    ...
    if (modalState.type === 'BOOK') {
      if (!bookingName.trim() || !bookingDept.trim() || !bookingContent.trim()) {
        alert('예약자명, 부서, 예약 내용을 모두 입력해주세요.');
        return;
      }
            ...
            <div className="py-4 text-slate-600 dark:text-slate-300">
              ...
              {modalState.type === 'BOOK' && (
                <div className="flex flex-col gap-3 mt-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">예약자</label>
                    <input
                      type="text"
                      value={bookingName}
                      onChange={(e) => setBookingName(e.target.value)}
                      className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none"
                      placeholder="이름 입력"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">예약 내용</label>
                    <input
                      type="text"
                      value={bookingContent}
                      onChange={(e) => setBookingContent(e.target.value)}
                      className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none"
                      placeholder="회의 주제 또는 목적"
                    />
                  </div>
                </div>
              )}
                        ...
                        <td key={time} className="px-1 py-2 align-middle h-16">
                          {isEditing ? (
                            ...
                            <div
                              ...
                              className={`w-full h-full min-h-[4rem] rounded-lg border flex flex-col items-center justify-center transition-all shadow-sm relative z-[1] cursor-pointer p-1 overflow-hidden ${isMyBooking
                              ...
                              title={isAdmin ? "관리자 권한으로 예약 취소 가능" : isMyBooking ? "클릭하여 예약 취소" : `${booking.department} ${booking.userName}\n${booking.content || ''}`}
                            >
                              <div className={`text-[10px] font-bold leading-tight text-center w-full truncate ${isMyBooking ? 'text-indigo-700 dark:text-indigo-300' : isAdmin ? 'text-red-600 dark:text-red-400' : 'text-slate-600 dark:text-slate-400'}`}>
                                {booking.department} {booking.userName}
                              </div>
                              {booking.content && (
                                <div className={`text-[9px] mt-0.5 text-center w-full truncate px-1 ${isMyBooking ? 'text-indigo-600 dark:text-indigo-400' : isAdmin ? 'text-red-500 dark:text-red-400' : 'text-slate-500 dark:text-slate-500'}`}>
                                  {booking.content}
                                </div>
                              )}
                            </div>
